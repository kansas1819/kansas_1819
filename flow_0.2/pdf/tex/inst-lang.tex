This package contains OMR Sheets and Buffer Sheets to be used only for the {\bi Languages} portion of CSAS. Buffer Sheets are provided
for use only in {\bi emergency situations}. The buffer sheets are different for {\bi Classes(4,5)} and {\bi Classes(6,7,8,9)}.
Use {\bi Buffer Sheet L1} for {\bi Classes(4,5)} and {\bi Buffer Sheet L2} for {\bi Classes(6,7,8,9)}.
The items {\bi a}, {\bi b}, {\bi c} and {\bi d} for {\bi Clases(4,5)} and items {\bi a}, {\bi b}, {\bi c}, {\bi d} and {\bi e} for {\bi Classes(6,7,8,9)} on the buffer sheet need to be filled properly.
...
\subject{After the test}
    \vskip-1ex
\startitemize[n,packed, joinedup, nowhite]
  \item Make sure that the "Question Paper Version" portion of the OMR sheet is filled.
