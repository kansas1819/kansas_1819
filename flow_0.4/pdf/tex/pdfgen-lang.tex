...
\setupheadertexts[text][{\tfc Cover Sheet}] [{\tfc Languages(2018)}]
...
local function cschooltable(data)
   ...
   for i,v in ipairs(data) do
      if i <= 3 then
         ...
         tex[#tex+1] = '\\eTABLE'
         tex[#tex+1] = '\\mycpar'
         tex[#tex+1] = '\\externalfigure[img/lang.pdf]'
...
local function cschooltableo(data)
   tex[#tex+1] = '\\startmycpar'
   tex[#tex+1] = '\\bTABLE[frame=off]'
   for i,v in ipairs(data) do
      tex[#tex+1] = '\\bTR '
...
local function genCover()
   ...
   --cols = clobberCols(cols)
   --inspect(tablerows)
   --tex[#tex+1] = '\\input cover/inst-lang.tex'
   --sheettable(cols,tablerows)
   tex[#tex+1] = '\\blank[1*big] \\hskip3em' 
   --tex[#tex+1] = '{\\startcombination[distance=3cm]'
   tex[#tex+1] = '\\startalignment[center]'            
   tex[#tex+1] = '\\blank {{\\bf Table 1~~} OMR Sheets Enclosed}\\blank {\\getbuffer[a]}\\blank[2*big]'
   tex[#tex+1] = '\\hrule\\blank {\\bf For the use of HM}\\hfill\\blank'
   tex[#tex+1] = '\\stopalignment'
   tex[#tex+1] = '\\startalignment[center]'                          
   tex[#tex+1] = 'Fill {\\bf Table 2} after the Assessment. Sign the sheet and place it at the top of the {\\bi Languages} OMR bundle.\\hfill\\blank[2*big]'
   
   tex[#tex+1] = '{\\bf Table 2~~} OMR Sheets Returned(Filled + Absent)\\blank \\getbuffer[b] \\blank \\noindent \\blank'
   tex[#tex+1] = '\\stopalignment \n \\blank[1*big] \n'          
   --inspect(tex)
...
local function cPage(row,cols)
   ...
   if names['STANDARD'] == "4" or names['STANDARD'] == "5" then
      context('\\defineoverlay[backgroundfigure][\\overlayfigure{../45-lang-sig.pdf}]')
      context('\\setupbackgrounds[page][background=backgroundfigure]')
   else
      context('\\defineoverlay[backgroundfigure][\\overlayfigure{../69-lang-sig.pdf}]')
      context('\\setupbackgrounds[page][background=backgroundfigure]')
   end
   page = [[  \noindent \startmypar
       \bTABLE[frame=off]
