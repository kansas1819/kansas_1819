...
def _xtract(pdf,page,npdf,tmpdir):
...
def xtract(pdf,page,npdf,tmpdir):
    cmd = ['cpdf', pdf, str(page),f'{tmpdir}/{npdf}']
    print(' '.join(cmd))
    p = subprocess.run(' '.join(cmd), shell=True, check=True)
...
def pdfTypes(df):
    ...
    for i,chunk in enumerate(chunks (std45.npdf.values.to_list(), 50)):
        pdfd['std45{}'.format(i)] = chunk
    std69 = df[df.STANDARD.isin([6,7,8,9,0])]
    for i,chunk in enumerate(chunks (std69.npdf.values.to_list(), 50)):
        pdfd['std69{}'.format(i)] = chunk
    return pdfd
        
def _extractPages(opts):
    df = opts.df
...
def extractPages(opts):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        os.makedirs(dirname, exist_ok=True)
        #cmd = ['pdftk', '{}/*-[45]-*.pdf'.format(tmpdir), 'cat', 'output', opath]
        cmd = ['cpdf', '-merge', '{tmpdir}/*-[45]-*.pdf', '-o', opath]
        subprocess.run(' '.join(cmd), shell=True, check=True)
        
        stmppdf = '{}/forms/45-{}-stamp.pdf'.format(opts.flowdir, papertype)
        opaths = '{}/{}/{}/45/45-stamp.pdf'.format(testdir, district, papertype)
        #cmd = ['pdftk', opath, 'multistamp', stmppdf, 'output', opaths]
        cmd = ['cpdf', '-stamp-on', stmppdf, opath, '-o', opaths]
        subprocess.run(' '.join(cmd), shell=True, check=True)
        dbdpath = os.path.dirname(opaths)
        dbspath = '{}/forms/{}/45/db'.format(opts.flowdir, papertype)
        cmd = 'cp -r {} {}'.format(dbspath, dbdpath)
        ...
        os.makedirs(dirname, exist_ok=True)
        #cmd = ['pdftk', '{}/*-[6789]-*.pdf'.format(tmpdir), 'cat', 'output', opath]
        cmd = ['cpdf', '-merge', '{tmpdir}/*-[6-9]-*.pdf', '-o', opath]
        subprocess.run(' '.join(cmd), shell=True, check=True)
