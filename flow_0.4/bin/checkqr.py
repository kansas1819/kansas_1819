...
def readQrCode(img):
    ...
    '''
    data = None
    cmdstr = 'zbarimg -Sdisable -Sqr.enable -q {}'.format(img)
    try:
...
def qrDatatoDf(data):
    ...
    for x in data:
        ...
        d.append([int(x) for x in [pagenum, sid, std, cid]])
    cols = ['PAGE', 'SCHOOL_ID','CHILD_ENROLL_NO','STANDARD']
    df = pd.DataFrame(d, columns=cols)
    df.sort_values(['PAGE'], ascending=True).to_csv("qrdata.csv")

def verifyQR(args):
    cols = ['SCHOOL_ID', 'STANDARD', 'CHILD_ENROLL_NO']
    #print qdf.dtypes
...
def main():
    ...
    qrdf = (qrDatatoDf(readQrCodes(args)))
    verifyQR(args)
    print ('Verified QR.')
